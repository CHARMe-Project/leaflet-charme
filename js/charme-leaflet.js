(function (root, factory) {
     ...
     */
    function CharmeAnnotator(charmeUrl, charmeClientId, map, strabon = true) {
    ...
    CharmeAnnotator.prototype.toggleAnnotations = function () {
        if (this.annotationsOn) {
            ...
            fetch(this.charmeUrl + 'sparql?format=' + format + '&query=' + encodeURIComponent(_getQuery(this.datasetUri, this.datasetVar)), {
                'headers': {
                    ...
                    L.geoJson(resp, {
                        ...
                        onEachFeature: function (feature, layer) {
                            if (feature.properties) {
                                var popupText = that._getPopupText(feature.properties);
                                layer.bindPopup(popupText);
                            }
                        }
                    }).addTo(that.annotationsGroup);
                } else {
                    var features = resp.results.bindings;
                    for (var i = 0; i < features.length; i++) {
                        var feature = features[i];
                        var wkt = new Wkt.Wkt(feature.wkt.value);
                        var obj = wkt.toObject(that.map.defaults);

                        /*
                         * Create a properties object in the same format as that
                         * used for the Strabon/GeoJSON case
                         */
                        var props = {};
                        props.text = feature.text.value;
                        props.time = feature.time.value;
                        props.email = feature.email.value;
                        props.firstname = feature.firstname.value;
                        props.surname = feature.surname.value;
                        props.account = feature.account.value;
                        popupText = that._getPopupText(props);
                        obj.bindPopup(popupText);
                        obj.addTo(that.map);
                    }
    ...
    CharmeAnnotator.prototype._getPopupText = function (properties) {
        var popupText = ''
        if (this.formatAnnotation && this.formatAnnotation instanceof Function) {
            /*
             * We can either use a user-supplied
             * format for the annotation...
             */
            popupText = this.formatAnnotation(properties);
        } else {
            /*
             * ...or use this default format
             */
            if (properties.text) {
                popupText += properties.text + '<br><br>';
            }
            if (properties.time) {
                popupText += 'Annotated at ' + properties.time + ' by:<br>';
            }
            if (properties.email) {
                popupText += '<a href="mailto:' + properties.email + '">';
            }
            if (properties.firstname && properties.surname) {
                popupText += '<b>' + properties.firstname + ' ' + properties.surname + '</b><br>';
            }
            if (properties.email) {
                popupText += '</a>';
            }
