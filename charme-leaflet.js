var MINI = require('minified');
var _ = MINI._,
    $ = MINI.$,
    $$ = MINI.$$,
    EE = MINI.EE,
    HTML = MINI.HTML;

var charmeUrl = 'http://192.168.56.102/';
var charmeOAuth = new JSO({
    providerID: 'charme-local',
    client_id: 'e6c205f28f25a9f11b71',
    redirect_uri: [location.protocol, '//', location.host, location.pathname].join(''),
    authorization: charmeUrl + 'oauth2/authorize'
});

var datasetUri = 'http://dataset/uri';
var datasetVar = 'selectedVar';
var map;
var drawControl;

var annotationsGroup;
var annotationsOn = false;

var loggedInCallback;
var loggedOutCallback;
var formatAnnotation;

var token;

document.addEventListener('DOMContentLoaded', function () {
    // TODO These should be args to the initial object...
    // Set up maps
    var layer1 = L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: 'Map data &copy; <a href="http://www.osm.org">OpenStreetMap</a>',
        maxZoom: 18
    });

    map = L.map('map', {
        layers: [layer1],
        center: [55, -3.5],
        zoom: 6
    });

    charmeOAuth.callback();
    var tokenObj = charmeOAuth.checkToken();
    // We are already logged into a CHARMe node.
    // Either we have just been redirected back, or we were already logged in from a previous session...
    if (tokenObj) {
        token = tokenObj['access_token'];

        // Get the user details and call the logged-in callback (if present)
        var headers = {
            'Authorization': 'Token ' + token,
        };
        $.request('get', charmeUrl + 'token/userinfo', null, {
            'headers': headers
        }).then(function (resp) {
            userdetails = JSON.parse(resp);
            if (loggedInCallback && loggedInCallback instanceof Function) {
                loggedInCallback(userdetails);
            }
        });

        annotationsGroup = new L.FeatureGroup();
        map.addLayer(annotationsGroup);

        // Add the commenting tools to the map
        var featureGroup = new L.FeatureGroup();
        map.addLayer(featureGroup);
        // Set the title to show on the polygon button
        L.drawLocal.draw.toolbar.buttons.polygon = 'Comment on a general region of data';
        L.drawLocal.draw.toolbar.buttons.rectangle = 'Comment on a rectangular region of data';
        L.drawLocal.draw.toolbar.buttons.marker = 'Comment on data at a point';

        drawControl = new L.Control.Draw({
            position: 'topleft',
            draw: {
                polyline: false,
                circle: false,
                marker: true,
                polygon: true,
                rectangle: true
            }
        });
        map.addControl(drawControl);


        // Wire up what happens when we add comments
        map.on('draw:created', function (e) {
            var tempMarker = featureGroup.addLayer(e.layer);
            var popupContent = '<form id="form">' +
                'Comment:<br>' +
                '<textarea id="comment" name="comment" cols="35" rows="5" wrap="soft" form="form"></textarea>' +
                '<input id="submit" type="submit" value="Submit">' +
                '</form>';

            popupform = $('#form');
            var popup = tempMarker.bindPopup(popupContent, {
                keepInView: true,
                closeButton: true
            });

            popup.openPopup();
            popup.on('popupclose', function (e) {
                featureGroup.removeLayer(e.layer)
            });

            $('#submit').on('click', function () {
                var comment = $$("#comment").value;
                var location = _getLocationString(e.layer);
                var ttl = _getTurtle(datasetUri, datasetVar, location, comment);

                if (!token) {
                    jso.getToken(function (tkn) {
                        token = tkn.access_token;
                    });
                }

                var headers = {
                    'Content-Type': 'text/turtle',
                    'Authorization': 'Token ' + token,
                };
                $.request('post', charmeUrl + 'insert/annotation', ttl, {
                    'headers': headers
                }).then(function () {
                    if (annotationsOn) {
                        // Switch annotations off and on again to refresh
                        toggleAnnotations();
                        toggleAnnotations();
                    }
                }).error(function () {
                    console.log('Problem creating annotation');
                });
                featureGroup.removeLayer(e.layer);
            });
        });
    }
});

function setDatasetDetails(uri, variable) {
    datasetUri = uri;
    datasetVar = variable;
}

function charmeLogin() {
    charmeOAuth.getToken();
}

function charmeLogout() {
    charmeOAuth.wipeTokens();
    token = undefined;
    if (loggedOutCallback && loggedOutCallback instanceof Function) {
        loggedOutCallback();
    }
    map.removeControl(drawControl);
}

function setLoggedInCallback(f) {
    loggedInCallback = f;
}

function setLoggedOutCallback(f) {
    loggedOutCallback = f;
}

function setFormatAnnotation(f) {
    formatAnnotation = f;
}

function toggleAnnotations() {
    if (annotationsOn) {
        annotationsGroup.clearLayers();
        annotationsOn = false;
    } else {
        var params = {
            'query': _getQuery(datasetUri, datasetVar),
            'format': 'GeoJSON'
        };
        var headers = {
            'Accept': 'application/json'
        }
        $.request('get', 'http://192.168.56.102:8080/strabonendpoint/Query', params, {
            'headers': headers
        }).then(function (resp) {
            L.geoJson(JSON.parse(resp), {
                onEachFeature: function (feature, layer) {
                    if (feature.properties) {
                        var p = feature.properties;
                        var popupText = ''
                        if (formatAnnotation && formatAnnotation instanceof Function) {
                            popupText = formatAnnotation(p);
                        } else {
                            if (p.text) {
                                popupText += p.text + '<br><br>';
                            }
                            if (p.time) {
                                popupText += 'Annotated at ' + p.time + ' by:<br>';
                            }
                            if (p.email) {
                                popupText += '<a href="mailto:' + p.email + '">';
                            }
                            if (p.firstname && p.surname) {
                                popupText += '<b>' + p.firstname + ' ' + p.surname + '</b><br>';
                            }
                            if (p.email) {
                                popupText += '</a>';
                            }
                        }
                        layer.bindPopup(popupText);
                    }
                }
            }).addTo(annotationsGroup);
        });
        annotationsOn = true;
    }
}

function isAnnotationsOn() {
    return annotationsOn;
}

function _getQuery(datasetUri, datasetVariable) {
    var query = 'PREFIX charme: <http://purl.org/voc/charme#> ' +
        'PREFIX oa: <http://www.w3.org/ns/oa#> ' +
        'PREFIX geo: <http://www.opengis.net/ont/geosparql#> ' +
        'PREFIX geof: <http://www.opengis.net/def/function/geosparql/> ' +
        'PREFIX cnt: <http://www.w3.org/2011/content#> ' +
        'PREFIX foaf: <http://xmlns.com/foaf/0.1/> ' +
        'SELECT ?wkt ?text ?firstname ?surname ?email ?time ?account ' +
        'WHERE { ' +
        '    ?anno oa:hasBody ?body . ' +
        '    ?anno oa:annotatedBy ?authorUri . ' +
        '    ?anno oa:annotatedAt ?time . ' +
        '    ?authorUri foaf:givenName ?firstname . ' +
        '    ?authorUri foaf:familyName ?surname . ' +
        '    ?authorUri foaf:mbox ?email . ' +
        '    ?authorUri foaf:accountName ?account . ' +
        '    ?body cnt:chars ?text . ' +
        '    ?anno oa:hasTarget ?target . ' +
        '    ?target oa:hasSelector ?selector . ' +
        '    ?selector charme:hasSpatialExtent ?sp . ' +
        '    ?sp geo:hasGeometry ?geometry . ' +
        '    ?geometry geo:asWKT ?wkt . ' +
        '    ?datasetUri a charme:dataset . ' +
        '    ?selector charme:hasVariable ?variableUri . ' +
        '    ?variableUri charme:hasInternalName ?variableName . ' +
        'FILTER(?variableName="' + datasetVariable + '") . ' +
        'FILTER(str(?datasetUri)="' + datasetUri + '") . ' +
        '} ';
    return query;
}

function _getLocationString(layer) {
    if (layer instanceof L.Marker) {
        return 'POINT (' + layer._latlng['lng'] + ' ' + layer._latlng['lat'] + ')'
    } else if (layer instanceof L.Polyline) {
        // Reverse order of points to ensure they are anti-clockwise
        var poly = 'POLYGON ((';
        for (var i = layer._latlngs.length - 1; i >= 0; i--) {
            // Be careful here - may need to reverse the order for Strabon
            var latlng = layer._latlngs[i];
            poly = poly + latlng['lng'] + ' ' + latlng['lat'] + ', ';
        }
        poly = poly + layer._latlngs[layer._latlngs.length - 1]['lng'] + ' ' + layer._latlngs[layer._latlngs.length - 1]['lat'] + '))';
        console.log(poly);
        return poly;
    }
}

function _getTurtle(datasetUri, datasetVar, location, comment) {
    var dateTime = new Date();
    var ttl = '@prefix chnode: <http://localhost/> .' +
        '@prefix charme: <http://purl.org/voc/charme#> .' +
        '@prefix oa: <http://www.w3.org/ns/oa#> .' +
        '@prefix prov: <http://www.w3.org/ns/prov#> .' +
        '@prefix xsd: <http://www.w3.org/2001/XML-Schema#> .' +
        '@prefix geo: <http://www.opengis.net/ont/geosparql#> .' +
        '@prefix cnt: <http://www.w3.org/2011/content#> .' +
        '@prefix dc: <http://purl.org/dc/elements/1.1/> .' +
        '@prefix dctypes: <http://purl.org/dc/dcmitype/> .' +
        '<chnode:annoID> a oa:Annotation ;' +
        '    oa:annotatedAt "' + dateTime.toISOString() + '" ;' +
        '    oa:hasTarget <chnode:targetID> ;' +
        '    oa:hasBody <chnode:bodyID> ;' +
        '    oa:motivatedBy oa:linking .' +
        '<chnode:targetID> a charme:DatasetSubset ;' +
        '    oa:hasSource <' + datasetUri + '> ;' +
        '    oa:hasSelector <chnode:subsetSelectorID> .' +
        '<' + datasetUri + '> a charme:dataset .' +
        '<chnode:subsetSelectorID> a charme:SubsetSelector ;' +
        '    charme:hasVariable <chnode:variableID-01> ;' +
        '    charme:hasSpatialExtent <chnode:spatialExtentID-01> .' +
        '<chnode:bodyID> a cnt:ContentAsText, dctypes:Text ;' +
        '    cnt:chars "' + comment + '" ;' +
        '    dc:format "text/plain" .    ' +
        '<chnode:variableID-01> a charme:Variable ;' +
        '    charme:hasInternalName "' + datasetVar + '" .' +
        '<chnode:spatialExtentID-01> a charme:SpatialExtent ;' +
        '    geo:hasGeometry <chnode:geometryID-01> .' +
        '<chnode:geometryID-01> a geo:Geometry ;' +
        '    geo:asWKT "' + location + '"^^geo:wktLiteral .';
    return ttl;
}